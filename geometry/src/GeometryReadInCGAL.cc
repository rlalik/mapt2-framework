...
GeometryReadInCGAL::GeometryReadInCGAL (UserCGALGeometry *geo, string filename_)
{
    geometry = geo;
    repeat = false;
    filename = filename_;
...
void GeometryReadInCGAL::readline (string l)
{
  string line = l;
...
void GeometryReadInCGAL::loadPart (string path_, string material_, string detector_, string color_)
{
    CADPartCGAL* part = new CADPartCGAL(geometry->getDrawable());
    part->Init(path_,material_,color_);
    geometry->addPart(part);
}

void GeometryReadInCGAL::loadPart (string path_, string material_, string detector_, string color_, string start_, string end_)
{
    ...
    if (detector == 1) // fiber
    {
        ...
        for (int i = start; i <= end; i++)
        {
            ...
            CADFiberCGAL* part = new CADFiberCGAL(geometry->getDrawable());
